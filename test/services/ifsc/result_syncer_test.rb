...
module Ifsc
  class ResultSyncerTest < ActiveSupport::TestCase
    ...
    test "syncs events from season data" do
      data = JSON.parse(Rails.root.join("test/fixtures/files/ifsc_seasons_response.json").read)
    ...
    test "syncs round results from category data" do
      ...
      data = JSON.parse(Rails.root.join("test/fixtures/files/ifsc_category_results_response.json").read)
